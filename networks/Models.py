...
class MainModel(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.lstm = nn.LSTM(436, 384, batch_first=True)
        self.fc1 = nn.Linear(1088 + 108, 512)
        self.fc2 = nn.Linear(512 + 384, 512)
    ...
    def forward(self, x: torch.Tensor, history: torch.Tensor):
        if history.shape[1] > 0:
            ...
            history_out = torch.zeros((x.shape[0], 384))
        x = self.fc1(x)
        x = torch.relu(x)
        x = self.fc2(torch.hstack([x, history_out]))
        x = torch.relu(x)
        x = self.fc3(x)
        x = torch.relu(x)
        x = self.fc4(x)
